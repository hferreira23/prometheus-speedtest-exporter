...
while IFS=$'\t' read -r servername serverid latency jitter packetloss download upload downloadedbytes uploadedbytes shareurl; do
    printMetric "speedtest_servername_info" "Server Name" "gauge" "1" "{servername=\"$servername\"}"
    printMetric "speedtest_serverid_info" "Server ID" "gauge" "1" "{serverid=\"$serverid\"}"
    ...
    printMetric "speedtest_shareurl_info" "Share URL" "gauge" "1" "{shareurl=\"$shareurl\"}"
